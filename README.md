...
OBS 媒体源默认用 **UDP** 传输视频，UDP 容易被防火墙或跨网段阻断。若手机与 PC 不在同一网段（如手机 192.168.108.x、PC 192.168.100.x），极易出现此问题。

**方案 1：VLC + 窗口捕获（推荐）**  
1. 用 VLC 强制 TCP 播放：`vlc --rtsp-tcp rtsp://手机IP:8554/live`  
2. 在 OBS 中添加「窗口捕获」，选择 VLC 窗口

**方案 2：ffplay + 窗口捕获**  
1. 命令行运行：`ffplay -rtsp_transport tcp rtsp://手机IP:8554/live`  
2. OBS 添加「窗口捕获」，选择 ffplay 窗口

**方案 3：放行 UDP 端口**  
- 在 PC 防火墙中放行入站 UDP 5004–5010（来源为手机所在网段），或关闭防火墙测试

**方案 4：同网段**  
- 让手机和 PC 处于同一网段（如都连同一 WiFi，且非访客网络）
